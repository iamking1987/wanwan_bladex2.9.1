...
import org.springzhisuan.core.tool.function.CheckedFunction;
...
import org.springzhisuan.core.tool.utils.Unchecked;
...
@Slf4j
@AllArgsConstructor
public class ZhisuanConverter implements Converter {
	 ...
	 * @param fieldName 目标的field名，原为 set 方法名，ZhisuanBeanCopier 里做了更改
	 * @return {Object}
	 */
	@Override
	@Nullable
	public Object convert(Object value, Class target, final Object fieldName) {
		...
		try {
			TypeDescriptor targetDescriptor = ZhisuanConverter.getTypeDescriptor(targetClazz, (String) fieldName);
			// 1. 判断 sourceClazz 为 Map
			if (Map.class.isAssignableFrom(sourceClazz)) {
				return ConvertUtil.convert(value, targetDescriptor);
			} else {
				TypeDescriptor sourceDescriptor = ZhisuanConverter.getTypeDescriptor(sourceClazz, (String) fieldName);
				return ConvertUtil.convert(value, sourceDescriptor, targetDescriptor);
			}
		} catch (Throwable e) {
			log.warn("ZhisuanConverter error", e);
			return null;
		}
	}
